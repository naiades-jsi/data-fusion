...
def RunBatchFusionOnce():
    ...
    file_json = open(f'{folder}/features_alicante_salinity_EA003_36_conductivity_hourly.json', 'r')
    ...
    file_json = open(f'alicante_salinity_salinity_EA003_36_conductivity_hourly_config.json', 'w')
    ...
    if(update_outputs):
      file_json = open(f'{folder}/features_alicante_salinity_EA003_36_conductivity_hourly.json', 'a')
      ...
      for j in range(t.shape[0]):
          ...
          output_topic = "features_alicante_salinity_EA003_36_conductivity_hourly"
         # Start Kafka producer
          if((all(isinstance(x, (float, int)) for x in fv[j])) and (not np.isnan(fv[j]).any())):
            future = producer.send(output_topic, output)
            output_topic = "features_alicante_salinity_EA003_36_conductivity_hourly_1"
            future = producer.send(output_topic, output)
            output_topic = "features_alicante_salinity_EA003_36_conductivity_hourly_2"
            future = producer.send(output_topic, output)
