...
def RunBatchFusionOnce(location):
    ...
    file_json = open(f'{folder}/features_alicante_{location}_anomaly_Gan.json', 'r')
    ...
    file_json = open(f'alicante_{location}_anomaly_Gan_config.json', 'w')
    ...
    if(update_outputs):
      ...
      for j in range(t.shape[0]):
          if(not np.isnan(fv[j]).any()):
            ...
            with open(f'{folder}/features_alicante_{location}_anomaly_Gan.json', 'a') as file_json:
              file_json.write((json.dumps(fv_line) + '\n' ))

      for j in range(t.shape[0]):
          if(not np.isnan(fv[j]).any()):
            output = {"timestamp":int(t[j].astype('uint64')/1000000), "ftr_vector":list(fv[j])}
            output_topic = f'features_alicante_{location}_anomaly_Gan'
...
for l in measurements_analog:
  schedule.every().hour.do(lambda: RunBatchFusionOnce(l))
